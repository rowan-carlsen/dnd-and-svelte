<script lang="ts">
	...
	const { randomValues } = missionData;
	...
	let xPos = $state(0),
		...
		rotation = $state(0),
		ready = $state(false);
	const { isPlanet }: { isPlanet: boolean } = missionData[selectedMission],
		missionNumber = parseInt(selectedMission.split(" ")[1]),
		planetImage = asset(`/${isPlanet ? "planets" : "locations"}/mission-${missionNumber}.png`);
...
<style>
	...
	#scan {
		...
		inset: 0;
		background-image:
			repeating-linear-gradient(transparent, transparent 48px, #16f529 48px, #16f529 50px),
